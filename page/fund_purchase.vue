<template>
	<section class="fund_purchase_page">
		...
		<section class="fund_purchase_page_totals" aria-label="基金部位與損益總覽">
			...
			<article class="fund_purchase_page_total normal_shadow"><span>{{ activeFund.name }}總投入本金</span><strong>{{
					formatTwd(activeFundTotalPrincipal) }}</strong><small>包含待補資料的投入本金</small></article>
...
module.exports = {
	...
	computed: {
		...
		activeFundTotalPrincipal() { return this.getFundPurchaseRecords(this.activeFundKey).reduce((total, record) => { const principal = Number(record?.principal); return total + (Number.isFinite(principal) && principal > 0 ? principal : 0); }, 0); },
