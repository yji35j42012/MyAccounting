	...
	<div class="fund">
		...
		<section class="fund_hero normal_shadow">
			<div class="fund_hero_main">
				...
				<div class="fund_hero_nav">
					...
					<small :class="['fund_nav_refresh_status', activeNav.navError ? 'is-error' : '']">{{ navStatus
					}}</small>
					<small class="fund_cache_status">{{ navTimingStatus }}</small>
					<button type="button" class="fund_cache_clear_button" @click="clearFundNavCache"
						aria-label="清除本機淨值快取">清除本機淨值快取</button>
					<a class="fund_source_link" :href="activeFund.sourceUrl" target="_blank"
						rel="noopener noreferrer">查看淨值來源</a>
				</div>
			</div>
			<aside class="fund_hero_meta fund_hero_contribution_card">
				<span class="fund_meta_label">公開前十大對基金淨值估計貢獻</span>
				<strong :class="getChangeClass(holdingsFundContributionPct)">{{
					formatPercent(holdingsFundContributionPct) }}</strong>
				<small class="fund_nav_contribution_detail"><span class="fund_nav_signal_line">前十大標準化日報酬（隨 Yahoo
						報價同步）：{{ formatPercent(holdingsWeightedChangePct) }}；</span><span
						class="fund_nav_signal_line">{{ holdingsSignalStatus }}{{ holdingsSignalTimestamp ? '：' : ''
						}}</span><span v-if="holdingsSignalTimestamp" class="fund_nav_signal_line">{{
							holdingsSignalTimestamp }}；</span><span class="fund_nav_signal_line">{{ holdingsSignalDateStatus
						}}</span></small>
				<button type="button" class="fund_full_refresh_button"
					:disabled="activeFullRefresh.isRefreshing || batchRefresh.isRefreshing"
					@click="clearAndRefreshActiveFundData" aria-label="清除目前基金全部快取並重新取得資料"><svg
						:class="['fund_refresh_icon', activeFullRefresh.isRefreshing ? 'is-spinning' : '']"
						viewBox="0 0 24 24" aria-hidden="true">
						<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
					</svg><span>{{ activeFullRefresh.isRefreshing ? '正在重新取得所有資料' : '清除快取並重新取得全部資料' }}</span></button>
				<small v-if="activeFullRefresh.message"
					:class="['fund_full_refresh_status', activeFullRefresh.isError ? 'is-error' : '']" role="status">{{
						activeFullRefresh.message }}</small>
				<button type="button" class="fund_batch_refresh_button" :disabled="batchRefresh.isRefreshing"
					@click="clearAndRefreshAllFundsData" aria-label="清除四檔基金全部快取並重新取得資料"><svg
						:class="['fund_refresh_icon', batchRefresh.isRefreshing ? 'is-spinning' : '']"
						viewBox="0 0 24 24" aria-hidden="true">
						<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
					</svg><span>{{ batchRefresh.isRefreshing ? '正在重整四檔基金' : '一鍵重整四檔基金' }}</span></button>
				<div class="fund_batch_nav_list" aria-label="四檔基金最新淨值">
					<article v-for="item in batchNavRows" :key="item.key" class="fund_batch_nav_row"><strong>{{
						item.name }}</strong><span>新臺幣 {{ formatPrice(item.nav) }}</span><small
							:class="['fund_batch_nav_change', getChangeClass(item.navChangePct)]">單日 {{
								formatPercent(item.navChangePct) }}</small><small
							:class="['fund_batch_nav_date', item.isStale ? 'is-stale' : '']">淨值日期 {{ item.navDate
							}}</small><small>{{ item.updatedTime }}</small></article>
				</div>
			</aside>
		</section>
		...
		<section class="fund_history normal_shadow">
			<div class="fund_section_head fund_history_head">
				...
				<div class="fund_history_meta"><span class="fund_asof">區間：{{ activeFund.historyRange }}</span><small
						:class="['fund_history_refresh_status', activeHistory.historyError ? 'is-error' : '']">{{
							historyStatus }}</small><small class="fund_cache_status">{{ historyTimingStatus
						}}</small><button type="button" class="fund_refresh_button fund_history_refresh_button"
						:disabled="activeHistory.isRefreshing" @click="refreshRecentHistoryNav(true)"
						aria-label="更新歷史淨值"><svg
							:class="['fund_refresh_icon', activeHistory.isRefreshing ? 'is-spinning' : '']"
							viewBox="0 0 24 24" aria-hidden="true">
							<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
						</svg><span>{{ activeHistory.isRefreshing ? '更新中' : '更新歷史淨值' }}</span></button></div>
			</div>
			<div class="fund_history_metrics">
				<div><span>區間最高</span><strong>{{ activeFund.historyHigh.value.toFixed(2) }}</strong><small>{{
					activeFund.historyHigh.date }}</small></div>
				<div><span>區間最低</span><strong>{{ activeFund.historyLow.value.toFixed(2) }}</strong><small>{{
					activeFund.historyLow.date }}</small></div>
			</div>
			<div class="fund_history_content">
				<div class="fund_history_list" aria-label="近期歷史淨值明細">
					...
					<article v-for="item in recentNavs" :key="item.date" class="fund_history_row">
						...
						<div class="fund_history_value_group"><strong>{{ item.value.toFixed(2) }}</strong><small
								:class="getChangeClass(item.changePct)">{{ formatPercent(item.changePct) }}</small>
						</div>
					</article>
				</div>
			</div>
			<p class="fund_history_source">資料來源：<a
					:href="activeFund.performanceSourceUrl || activeFund.sourceUrl + '?nav=navperformance'"
					target="_blank" rel="noopener noreferrer">官方淨值走勢</a>；最近五筆資料由<a :href="activeFund.historySourceUrl"
					target="_blank" rel="noopener noreferrer">公開基金淨值表</a>動態取得。</p>
		</section>

		<section class="fund_holdings normal_shadow">
			<div class="fund_section_head">
				<div class="fund_holdings_heading">
					<p class="fund_kicker">投資標的 <span class="fund_holdings_asof_inline">持股資料基準日：{{
						activeFund.holdingsDate }}</span></p>
					<h3>官方公開前十大持股</h3>
				</div>
				<div class="fund_holdings_controls">
					<div class="fund_holdings_update_meta"><small
							:class="['fund_holdings_refresh_status', activeHoldings.isRefreshing ? 'is-refreshing' : '', activeHoldings.holdingsError ? 'is-error' : '']">{{
								holdingsStatus }}</small><small class="fund_cache_status">{{ holdingsTimingStatus }}</small>
					</div>
					<button type="button" class="fund_refresh_button fund_holdings_refresh_button"
						:disabled="activeHoldings.isRefreshing" @click="refreshHoldings(true)"
						aria-label="更新官方公開持股"><svg
							:class="['fund_refresh_icon', activeHoldings.isRefreshing ? 'is-spinning' : '']"
							viewBox="0 0 24 24" aria-hidden="true">
							<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
						</svg><span>{{ activeHoldings.isRefreshing ? '更新中' : '更新持股' }}</span></button>
					<button type="button" class="fund_holdings_cache_clear_button"
						:disabled="activeHoldings.isRefreshing || activeQuote.isRefreshing"
						@click="clearActiveFundHoldingsCache" aria-label="一鍵清除持股快取">
						{{ activeHoldings.isRefreshing || activeQuote.isRefreshing ? '重新取得中' : '一鍵清除持股快取' }}
					</button>
				</div>
				<div class="fund_quote_controls">
					<div class="fund_section_meta fund_quote_meta">
						...
						<small :class="['fund_quote_hint', activeQuote.quoteError ? 'is-error' : '']">{{ quoteStatus
						}}</small>
						<small v-if="quoteCacheIncomplete" class="fund_quote_cache_warning" role="status">{{
							quoteCacheIncompleteMessage }}</small>
					</div>
					<button type="button" class="fund_refresh_button" :disabled="activeQuote.isRefreshing"
						@click="refreshYahooQuotes()" aria-label="更新 Yahoo 股價">
						<svg :class="['fund_refresh_icon', activeQuote.isRefreshing ? 'is-spinning' : '']"
							viewBox="0 0 24 24" aria-hidden="true">
							<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
						</svg>
			...
			<div class="fund_table_box">
				<table class="fund_table">
					...
					<tbody>
						<tr v-for="item in activeFund.holdings" :key="item.name">
							<td><span class="fund_rank">{{ item.rank }}</span></td>
							<td class="fund_company"><a v-if="getYahooQuoteUrl(item)" class="fund_company_link"
									:href="getYahooQuoteUrl(item)" target="_blank" rel="noopener noreferrer"
									:aria-label="`開啟 ${item.name} 的 Yahoo 股市報價`">{{ item.name }}</a><span v-else>{{
										item.name }}</span></td>
							<td class="fund_price"><strong>TWD {{ formatPrice(item.price) }}</strong><small
									class="fund_previous_close">前收盤 TWD {{ formatPrice(item.previousClose) }}</small>
							</td>
							<td class="fund_change">
								<strong :class="['fund_today_change', getChangeClass(item.changePct)]">
									...
									:class="getChangeClass(getQuoteChangeAmount(item))">{{
										formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else
									class="fund_flat">資料待驗證</small>
							</td>
							<td class="fund_weight"><strong>{{ item.weight.toFixed(2) }}%</strong></td>
						</tr>
					</tbody>
				</table>
			</div>
				...
				<article class="fund_mobile_holding" v-for="item in activeFund.holdings" :key="'mobile-' + item.name">
					<div class="fund_mobile_holding_top">
						<div class="fund_mobile_company_group"><span class="fund_rank">{{ item.rank }}</span><a
								v-if="getYahooQuoteUrl(item)" class="fund_company_link" :href="getYahooQuoteUrl(item)"
								target="_blank" rel="noopener noreferrer"
								:aria-label="`開啟 ${item.name} 的 Yahoo 股市報價`">{{ item.name }}</a><strong v-else>{{
									item.name }}</strong></div>
						<div class="fund_mobile_change"><strong :class="getChangeClass(item.changePct)"><span>{{
							isYahooQuoteVerified(item, activeQuote.savedAt) ? formatPercent(item.changePct) :
								'待確認' }}</span><em v-if="isFlatQuote(item, activeQuote.savedAt)"
									class="fund_flat_badge">平盤</em></strong><small
								v-if="isYahooQuoteVerified(item, activeQuote.savedAt)"
								:class="getChangeClass(getQuoteChangeAmount(item))">{{
									formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else
								class="fund_flat">資料待驗證</small></div>
					</div>
		...
		<section class="fund_insight_grid">
			<article v-for="insight in activeFund.insights" :key="insight.title" class="fund_insight normal_shadow">
				<p class="fund_kicker">{{ insight.kicker }}</p>
				<h3>{{ insight.title }}</h3>
				<p>{{ insight.text }}</p>
			</article>
		</section>
...
module.exports = {
	data() {
		return {
			...
			holdingsByFund: {
				taiwanTechnology: { holdingsUpdatedAt: '', fetchedAt: 0, cacheExpiresAt: 0, dataDate: '', cacheMode: '', isRefreshing: false, holdingsError: '', sourceName: '', sourceUrl: '' },
				taiwanDaba: { holdingsUpdatedAt: '', fetchedAt: 0, cacheExpiresAt: 0, dataDate: '', cacheMode: '', isRefreshing: false, holdingsError: '', sourceName: '', sourceUrl: '' },
				taiwanIntelligence: { holdingsUpdatedAt: '', fetchedAt: 0, cacheExpiresAt: 0, dataDate: '', cacheMode: '', isRefreshing: false, holdingsError: '', sourceName: '', sourceUrl: '' },
				fuhwaOmni: { holdingsUpdatedAt: '', fetchedAt: 0, cacheExpiresAt: 0, dataDate: '', cacheMode: '', isRefreshing: false, holdingsError: '', sourceName: '', sourceUrl: '' }
			},
			...
			fullRefreshByFund: {
				taiwanTechnology: { isRefreshing: false, isError: false, message: '' },
				taiwanDaba: { isRefreshing: false, isError: false, message: '' },
				taiwanIntelligence: { isRefreshing: false, isError: false, message: '' },
				fuhwaOmni: { isRefreshing: false, isError: false, message: '' }
			},
			batchRefresh: { isRefreshing: false },
			funds: [
				{
					...
					historyRange: '2026 / 06 / 30 — 2026 / 08 / 11',
					historyHigh: { value: 865.26, date: '2026 / 07 / 03' },
					historyLow: { value: 597.91, date: '2026 / 07 / 30' },
					summaryCards: [{
						label: '前十大列示比重', value: '53.80%',
						detail: '依官方逐檔權重加總'
					}, { label: '半導體業比重', value: '29.86%', detail: '僅以前十大官方產業分類計算' }],
					historyNav: [
						{ date: '2026-07-03', value: 865.26, changePct: 0.94 },
						{ date: '2026-07-06', value: 828.00, changePct: -4.31 },
						{ date: '2026-07-09', value: 789.20, changePct: 0.73 },
						{ date: '2026-07-13', value: 768.38, changePct: -2.64 },
						{ date: '2026-07-16', value: 751.18, changePct: -2.13 },
						{ date: '2026-07-20', value: 667.70, changePct: -2.90 },
						{ date: '2026-07-23', value: 741.53, changePct: 0.19 },
						{ date: '2026-07-27', value: 703.03, changePct: -0.13 },
						{ date: '2026-07-30', value: 597.91, changePct: -0.73 },
						{ date: '2026-08-03', value: 690.54, changePct: 5.91 },
						{ date: '2026-08-06', value: 751.72, changePct: 1.70 },
						{ date: '2026-08-11', value: 760.91, changePct: 1.42 }
					],
					holdings: [
						{ rank: 1, name: '華邦電子', symbol: '2344.TW', weight: 7.44, price: 177.00, changePct: -0.56, market: 'Yahoo · TWSE' },
						{ rank: 2, name: '旺矽', symbol: '6223.TWO', weight: 7.32, price: 6600.00, changePct: 4.51, market: 'Yahoo · TPEX' },
						{ rank: 3, name: '台積電', symbol: '2330.TW', weight: 6.37, price: 2415.00, changePct: 0.84, market: 'Yahoo · TWSE' },
						{ rank: 4, name: '國巨', symbol: '2327.TW', weight: 5.65, price: 602.00, changePct: -2.43, market: 'Yahoo · TWSE' },
						{ rank: 5, name: '台燿', symbol: '6274.TWO', weight: 5.38, price: 1600.00, changePct: -0.31, market: 'Yahoo · TPEX' },
						{ rank: 6, name: '台光電子', symbol: '2383.TW', weight: 5.34, price: 5730.00, changePct: 3.52, market: 'Yahoo · TWSE' },
						{ rank: 7, name: '穎崴', symbol: '6515.TW', weight: 4.49, price: 7170.00, changePct: 3.61, market: 'Yahoo · TWSE' },
						{ rank: 8, name: '創意電子', symbol: '3443.TW', weight: 4.48, price: 5130.00, changePct: 3.53, market: 'Yahoo · TWSE' },
						{ rank: 9, name: '聯電', symbol: '2303.TW', weight: 4.24, price: 123.00, changePct: 0.00, market: 'Yahoo · TWSE' },
						{ rank: 10, name: '奇鋐', symbol: '3017.TW', weight: 3.09, price: 2910.00, changePct: 5.43, market: 'Yahoo · TWSE' }
					],
					insights: [{
						kicker: '配置觀察', title: '以半導體與電子零組件為主軸',
						text: '以前十大公開標的的官方產業分類計算，半導體業合計 29.86%，電子零組件業合計 16.37%。其餘配置包含科技與電腦週邊，反映基金對台灣科技供應鏈的集中布局。'
					},
					{ kicker: '投資目標', title: '鎖定台灣科技產業', text: '官方資料說明，經理團隊以品質、成長與價值作為篩選基礎，並以台灣科技產業的成長前景為主要投資方向。' }
					]
				},
				{
					key: 'taiwanDaba', shortName: '安聯台灣大壩',
					name: '安聯台灣大壩基金-A類型', englishName: 'Allianz Global Investors Taiwan Fund', riskLevel: 'RR4',
					tags: ['單一國家股票型', '新臺幣計價', 'RR4'],
					nav: 313.43, navDate: '2026 / 08 / 11',
					holdingsDate: '2026 / 06 / 30', sourceUrl: 'https://tw.allianzgi.com/zh-tw/products-solutions/taiwan-onshore/allianz-global-investors-taiwan-fund-a-twd',
					historySourceUrl: 'https://fund.megabank.com.tw/w/wr/wr02_ACDD01-4101.djhtm',
					historyRange: '2026 / 06 / 30 — 2026 / 08 / 11',
					historyHigh: { value: 352.14, date: '2026 / 07 / 03' }, historyLow: { value: 246.18, date: '2026 / 07 / 30' },
					summaryCards: [
						{ label: '前十大列示比重', value: '58.50%', detail: '依官方逐檔權重加總' },
						{ label: '半導體業比重', value: '33.01%', detail: '僅以前十大官方產業分類計算' }
					],
					historyNav: [
						{ date: '2026-07-03', value: 352.14, changePct: 1.84 },
						{ date: '2026-07-06', value: 333.97, changePct: -5.16 }, { date: '2026-07-09', value: 323.50, changePct: 1.61 },
						{ date: '2026-07-13', value: 317.43, changePct: -1.88 }, { date: '2026-07-16', value: 306.20, changePct: -3.17 },
						{ date: '2026-07-20', value: 271.88, changePct: -2.73 }, { date: '2026-07-23', value: 304.54, changePct: 0.20 },
						{ date: '2026-07-27', value: 287.61, changePct: 0.04 }, { date: '2026-07-30', value: 246.18, changePct: -0.21 },
						{ date: '2026-08-03', value: 285.18, changePct: 6.20 }, { date: '2026-08-05', value: 306.42, changePct: 4.18 },
						{ date: '2026-08-06', value: 311.29, changePct: 1.59 }, { date: '2026-08-07', value: 303.65, changePct: -2.45 },
						{ date: '2026-08-10', value: 309.15, changePct: 1.81 }, { date: '2026-08-11', value: 313.43, changePct: 1.38 }],
					holdings: [{ rank: 1, name: '旺矽', symbol: '6223.TWO', weight: 12.17, price: null, changePct: null, market: 'Yahoo · TPEX' },
					{ rank: 2, name: '台燿', symbol: '6274.TWO', weight: 7.19, price: null, changePct: null, market: 'Yahoo · TPEX' },
					{ rank: 3, name: '穎崴', symbol: '6515.TW', weight: 7.11, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 4, name: '台光電子', symbol: '2383.TW', weight: 6.65, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 5, name: '台積電', symbol: '2330.TW', weight: 5.20, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 6, name: '欣興', symbol: '3037.TW', weight: 5.06, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 7, name: '信驊科技', symbol: '5274.TWO', weight: 4.75, price: null, changePct: null, market: 'Yahoo · TPEX' }, { rank: 8, name: '景碩科技', symbol: '3189.TW', weight: 3.78, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 9, name: '南電', symbol: '8046.TW', weight: 3.40, price: null, changePct: null, market: 'Yahoo · TWSE' }, { rank: 10, name: '台達電子', symbol: '2308.TW', weight: 3.19, price: null, changePct: null, market: 'Yahoo · TWSE' }], insights: [{ kicker: '配置觀察', title: '半導體與電子零組件占前十大核心', text: '以前十大公開標的的官方產業分類計算，半導體業合計 33.01%，電子零組件業合計 22.30%。配置同時涵蓋晶片測試、PCB 與電子材料等台灣科技供應鏈環節。' }, { kicker: '投資目標', title: '聚焦台灣上市與上櫃股票', text: '官方資料顯示，基金以台灣股票市場為主要投資範圍，並依品質、成長與價值的基本面研究架構篩選投資標的。' }]
				},
				{
	...
	computed: {
		...
		holdingsSignalStatus() { const signal = this.holdingsSignalsByFund?.[this.activeFundKey]; if (!Number.isFinite(signal?.weightedChangePct)) return '尚無本機紀錄；成功取得 Yahoo 報價後會自動儲存'; return signal.cacheMode === 'local' ? '已由本機快取載入' : '已隨 Yahoo 報價同步更新並儲存於本機'; },
		holdingsSignalTimestamp() { const signal = this.holdingsSignalsByFund?.[this.activeFundKey]; if (!Number.isFinite(signal?.weightedChangePct)) return ''; const updatedAt = signal.quoteUpdatedAt || (signal.savedAt ? this.formatQuoteTime(signal.savedAt) : '尚未標示'); const coverage = Number.isFinite(signal.quotedCount) && Number.isFinite(signal.holdingsCount) ? `${signal.quotedCount}/${signal.holdingsCount} 檔、權重 ${signal.totalWeight.toFixed(2)}%` : '公開持股'; return `${updatedAt}（${coverage}）`; },
		holdingsSignalDateStatus() { const signal = this.holdingsSignalsByFund?.[this.activeFundKey]; const updatedAt = signal?.quoteUpdatedAt || (signal?.savedAt ? this.formatQuoteTime(signal.savedAt) : ''); return this.getHoldingsQuoteDateStatus(updatedAt); },
	...
	methods: {
		...
		isFlatQuote(holding, referenceTime = Date.now()) { return this.isYahooQuoteVerified(holding, referenceTime) && Number.isFinite(holding?.changePct) && Math.abs(Number(holding.changePct)) < 0.005; },
		...
		getTiming() {
			...
			return {
				...
				getTaipeiCalendarDate(timestamp = Date.now()) { return formatDate(getTaipeiParts(timestamp)); },
		...
		clearActiveFundCacheRecords(fundKey) { ['nav', 'history', 'holdings', 'quotes', 'holdings-signal'].forEach(type => { try { localStorage.removeItem(this.getFundStorageKey(type, fundKey)); } catch { } }); },
		async clearAndRefreshFundData(fundKey) { const refreshState = this.fullRefreshByFund[fundKey]; const navState = this.navsByFund[fundKey]; const historyState = this.historiesByFund[fundKey]; const holdingsState = this.holdingsByFund[fundKey]; const quoteState = this.quotesByFund[fundKey]; const signalState = this.holdingsSignalsByFund[fundKey]; if (!refreshState || refreshState.isRefreshing) return false; refreshState.isRefreshing = true; refreshState.isError = false; refreshState.message = '正在清除本機快取並重新取得資料'; this.clearActiveFundCacheRecords(fundKey); this.quoteAutoSlotByFund[fundKey] = '';[navState, historyState, holdingsState, quoteState, signalState].forEach(state => { if (state) state.cacheMode = 'cleared'; }); if (navState) navState.navError = ''; if (historyState) historyState.historyError = ''; if (holdingsState) holdingsState.holdingsError = ''; if (quoteState) quoteState.quoteError = ''; const refreshStartedAt = Date.now(); try { const [navUpdated, historyUpdated, holdingsUpdated] = await Promise.all([this.refreshOfficialNav(true, fundKey), this.refreshRecentHistoryNav(true, fundKey), this.refreshHoldings(true, fundKey)]); const quoteUpdatedByHoldings = Boolean(quoteState?.cacheMode === 'remote' && Number(quoteState?.savedAt) >= refreshStartedAt && !quoteState?.quoteError); if (!quoteUpdatedByHoldings) await this.refreshYahooQuotes(fundKey); const quoteUpdated = Boolean(quoteState?.cacheMode === 'remote' && quoteState?.quotedCount > 0 && !quoteState?.quoteError); const historySucceeded = Boolean(historyUpdated !== false && !historyState?.historyError); const allUpdated = Boolean(navUpdated && historySucceeded && holdingsUpdated && quoteUpdated); refreshState.isError = !allUpdated; refreshState.message = allUpdated ? '本機快取已清除，淨值、歷史、持股與 Yahoo 報價皆已重新取得' : '已清除本機快取；部分資料更新失敗，畫面保留前次成功資料'; return allUpdated; } finally { refreshState.isRefreshing = false; } },
		async clearAndRefreshActiveFundData() { return this.clearAndRefreshFundData(this.activeFundKey); },
		async clearAndRefreshAllFundsData() { const batchState = this.batchRefresh; if (batchState.isRefreshing) return false; batchState.isRefreshing = true; try { const outcomes = await Promise.all(this.funds.map(fund => this.clearAndRefreshFundData(fund.key))); return outcomes.every(Boolean); } finally { batchState.isRefreshing = false; } },
		async clearActiveFundHoldingsCache() {
			...
			const signalState = this.holdingsSignalsByFund[fundKey];
		...
		applyHoldingsSnapshot(fundKey, snapshot, cacheMode = 'remote') {
			const targetFund = this.funds.find(fund => fund.key === fundKey);
			const holdingsState = this.holdingsByFund[fundKey];
			const rows = Array.isArray(snapshot?.holdings) ? snapshot.holdings.map((item, index) => ({ rank: Number(item?.rank) || index + 1, name: String(item?.name || '').trim(), weight: Number(item?.weight), industry: String(item?.industry || '').trim() })).filter(item => item.name && item.industry && Number.isFinite(item.weight) && item.weight >= 0 && item.weight <= 100).sort((left, right) => left.rank - right.rank).slice(0, 10) : [];
			if (!targetFund || !holdingsState || snapshot?.fundKey !== fundKey || !snapshot?.holdingsDate || !rows.length) return false;
			const previousDate = this.normalizeFundDate(targetFund.holdingsDate);
			const nextDate = this.normalizeFundDate(snapshot.holdingsDate);
			...
			targetFund.holdings = rows.map(item => {
				...
				const retainQuote = !holdingsChanged && prior?.symbol === symbol;
				return { ...item, symbol, price: retainQuote ? prior?.price ?? null : null, previousClose: retainQuote ? prior?.previousClose ?? null : null, priceChange: retainQuote ? prior?.priceChange ?? null : null, changePct: retainQuote ? prior?.changePct ?? null : null, market: symbol ? `Yahoo · ${symbol.endsWith('.TWO') ? 'TPEX' : 'TWSE'}` : '待確認 Yahoo 代號' };
			});
			targetFund.holdingsDate = holdingsDate;
			const totalWeight = rows.reduce((total, item) => total + item.weight, 0);
			const industryTotals = rows.reduce((result, item) => ({ ...result, [item.industry]: (result[item.industry] || 0) + item.weight }), {});
			const leadingIndustry = Object.entries(industryTotals).sort((left, right) => right[1] - left[1])[0];
			if (targetFund.summaryCards?.[0]) { targetFund.summaryCards[0].value = `${totalWeight.toFixed(2)}%`; targetFund.summaryCards[0].detail = '依官方公開前十大逐檔權重加總'; }
			if (targetFund.summaryCards?.[1] && leadingIndustry) { targetFund.summaryCards[1].label = `${leadingIndustry[0]}比重`; targetFund.summaryCards[1].value = `${Number(leadingIndustry[1]).toFixed(2)}%`; targetFund.summaryCards[1].detail = '僅以前十大官方產業分類計算'; }
			holdingsState.holdingsUpdatedAt = this.formatQuoteTime(snapshot.savedAt || snapshot.fetchedAt || Date.now());
			holdingsState.fetchedAt = Number(snapshot.fetchedAt) || Date.now();
			holdingsState.cacheExpiresAt = Number(snapshot.cacheExpiresAt) || holdingsState.fetchedAt + 24 * 60 * 60 * 1000;
			holdingsState.dataDate = nextDate;
			holdingsState.cacheMode = cacheMode;
			holdingsState.sourceName = String(snapshot.sourceName || holdingsState.sourceName || '官方公開持股');
			holdingsState.sourceUrl = String(snapshot.sourceUrl || holdingsState.sourceUrl || '');
			if (holdingsChanged) {
				const quoteState = this.quotesByFund[fundKey];
				if (quoteState) { quoteState.quoteUpdatedAt = ''; quoteState.quotedCount = 0; quoteState.cacheMode = ''; quoteState.quoteError = ''; }
				const signalState = this.holdingsSignalsByFund[fundKey];
				if (signalState) { signalState.weightedChangePct = null; signalState.fundContributionPct = null; signalState.totalWeight = 0; signalState.quotedCount = 0; signalState.holdingsCount = rows.length; signalState.quoteUpdatedAt = ''; signalState.cacheMode = ''; }
				if (cacheMode !== 'local') {
					try {
						localStorage.removeItem(this.getFundStorageKey('quotes', fundKey));
						localStorage.removeItem(this.getFundStorageKey('holdings-signal', fundKey));
					} catch { }
				}
			} else if (this.quotesByFund[fundKey]?.quotedCount) this.refreshHoldingsSignalFromCurrentQuotes(fundKey, cacheMode, true);
			return true;
		},
		hydrateHoldingsCache(fundKey) { const snapshot = this.readFundStorage('holdings', fundKey); return snapshot ? this.applyHoldingsSnapshot(fundKey, snapshot, 'local') : false; },
		isHoldingsCheckedToday(state) { const fetchedAt = Number(state?.fetchedAt); const timing = this.getTiming(); return Boolean(fetchedAt && timing?.getTaipeiCalendarDate && timing.getTaipeiCalendarDate(fetchedAt) === timing.getTaipeiCalendarDate(Date.now())); },
		async refreshHoldingsIfNeeded(fundKey = this.activeFundKey) { if (this.isHoldingsCheckedToday(this.holdingsByFund[fundKey])) return false; return this.refreshHoldings(false, fundKey); },
		...
		async refreshYahooQuotes(fundKey = this.activeFundKey) {
			...
			try {
				...
				const targetFund = this.funds.find(fund => fund.key === fundKey);
				if (!this.applyYahooQuoteSnapshot(fundKey, { ...snapshot, holdingsDate: targetFund.holdingsDate }, 'remote')) throw new Error('Yahoo 報價資料未通過完整性驗證');
				const coverage = this.getQuoteCacheCoverage(fundKey);
				if (!this.syncYahooQuoteAndHoldingsSignal(fundKey, snapshot.fetchedAt, coverage.quotedCount)) throw new Error('Yahoo 報價已更新，但估計貢獻同步失敗');
				if (snapshot.failedSymbols?.length || coverage.quotedCount < coverage.expectedCount) quoteState.quoteError = `部分報價更新失敗或未通過驗證（${coverage.quotedCount}/${coverage.expectedCount}）`;
			} catch (error) {
	...
	mounted() {
		console.info(`[現金流管理] fund_analysis.vue 版本：fund-analysis-v-2026.09.23-10`);
