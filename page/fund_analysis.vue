		...
		<section class="fund_history normal_shadow">
			...
			<section class="fund_holdings normal_shadow">
					...
					<div class="fund_mobile_holdings" aria-label="官方公開前十大持股">
						<article class="fund_mobile_holding" v-for="item in activeFund.holdings" :key="'mobile-' + item.name"><div class="fund_mobile_holding_top"><div class="fund_mobile_company_group"><span class="fund_rank">{{ item.rank }}</span><strong>{{ item.name }}</strong></div><div class="fund_mobile_change"><strong :class="getChangeClass(item.changePct)"><span>{{ isYahooQuoteVerified(item, activeQuote.savedAt) ? formatPercent(item.changePct) : '待確認' }}</span><em v-if="isFlatQuote(item, activeQuote.savedAt)" class="fund_flat_badge">平盤</em></strong><small v-if="isYahooQuoteVerified(item, activeQuote.savedAt)" :class="getChangeClass(getQuoteChangeAmount(item))">{{ formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else class="fund_flat">資料待驗證</small></div></div><div class="fund_mobile_holding_detail"><div class="fund_mobile_price fund_mobile_price_inline"><strong>TWD {{ formatPrice(item.price) }}</strong><small class="fund_previous_close">前收盤 TWD {{ formatPrice(item.previousClose) }}</small><small>{{ item.market }} ・ 比重 {{ item.weight.toFixed(2) }}%</small></div></div></article>
					</div>
...
<script>
const FUND_ANALYSIS_VERSION = 'fund-analysis-v1.5.30-2026.08.20';
	...
	computed: {
			...
			formatPrice(value) { return Number.isFinite(value) ? value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'; },
				...
				writeFundStorage(type, fundKey, snapshot) { try { localStorage.setItem(this.getFundStorageKey(type, fundKey), JSON.stringify({ ...snapshot, fundKey, savedAt: Date.now() })); } catch {} },
					...
					getQuoteCacheCoverage(fundKey = this.activeFundKey) { const targetFund = this.funds.find(fund => fund.key === fundKey); if (!targetFund) return { expectedCount: 0, quotedCount: 0 }; const expectedHoldings = targetFund.holdings.filter(holding => Boolean(holding?.symbol)); const quotedCount = expectedHoldings.filter(holding => Number.isFinite(holding?.price) && Number.isFinite(holding?.previousClose) && Number(holding.previousClose) > 0).length; return { expectedCount: expectedHoldings.length, quotedCount }; },
