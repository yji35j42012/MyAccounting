				...
				<h2 class="fund_title">{{ activeFund.name }}</h2>
					...
					<div class="fund_tags">
						...
						<aside class="fund_hero_meta fund_hero_contribution_card">
							...
							<small class="fund_nav_contribution_detail"><span class="fund_nav_signal_line">前十大標準化日報酬（隨 Yahoo 報價同步）：{{ formatPercent(holdingsWeightedChangePct) }}；</span><span class="fund_nav_signal_line">{{ holdingsSignalStatus }}{{ holdingsSignalTimestamp ? '：' : '' }}</span><span v-if="holdingsSignalTimestamp" class="fund_nav_signal_line">{{ holdingsSignalTimestamp }}；</span><span class="fund_nav_signal_line">{{ holdingsSignalDateStatus }}</span></small>
							...
							<div class="fund_batch_nav_list" aria-label="四檔基金最新淨值"><article v-for="item in batchNavRows" :key="item.key" class="fund_batch_nav_row"><strong>{{ item.name }}</strong><span>新臺幣 {{ formatPrice(item.nav) }}</span><small :class="['fund_batch_nav_change', getChangeClass(item.navChangePct)]">單日 {{ formatPercent(item.navChangePct) }}</small><small :class="['fund_batch_nav_date', item.isStale ? 'is-stale' : '']">淨值日期 {{ item.navDate }}</small><small>{{ item.updatedTime }}</small></article></div>
						</aside>
			</section>
			...
			<section class="fund_holdings normal_shadow">
					<div class="fund_section_head">
						...
						<div class="fund_holdings_controls">
							<div class="fund_holdings_update_meta"><small :class="['fund_holdings_refresh_status', activeHoldings.isRefreshing ? 'is-refreshing' : '', activeHoldings.holdingsError ? 'is-error' : '']">{{ holdingsStatus }}</small><small class="fund_cache_status">{{ holdingsTimingStatus }}</small></div>
							<button type="button" class="fund_refresh_button fund_holdings_refresh_button" :disabled="activeHoldings.isRefreshing" @click="refreshHoldings(true)" aria-label="更新官方公開持股"><svg :class="['fund_refresh_icon', activeHoldings.isRefreshing ? 'is-spinning' : '']" viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" /></svg><span>{{ activeHoldings.isRefreshing ? '更新中' : '更新持股' }}</span></button>
							<button type="button" class="fund_holdings_cache_clear_button" :disabled="activeHoldings.isRefreshing || activeQuote.isRefreshing" @click="clearActiveFundHoldingsCache" aria-label="一鍵清除持股快取">{{ activeHoldings.isRefreshing || activeQuote.isRefreshing ? '重新取得中' : '一鍵清除持股快取' }}</button>
						</div>
						...
						<aside v-if="quoteUnverifiedHoldings.length" class="fund_quote_missing" role="status" aria-live="polite"><strong>待補齊報價（{{ quoteUnverifiedHoldings.length }} 檔）</strong><span v-for="item in quoteUnverifiedHoldings" :key="`missing-${item.name}`">{{ item.name }}<small>{{ item.reason }}</small></span><p>請按「更新股價」重新取得最新 Yahoo 報價。</p></aside>
					<div class="fund_table_box">
						<table class="fund_table"><thead><tr><th>排名</th><th>投資標的</th><th>Yahoo 股價¹</th><th>今日漲跌²</th><th>比重</th></tr></thead>
							<tbody><tr v-for="item in activeFund.holdings" :key="item.name"><td><span class="fund_rank">{{ item.rank }}</span></td><td class="fund_company"><a v-if="getYahooQuoteUrl(item)" class="fund_company_link" :href="getYahooQuoteUrl(item)" target="_blank" rel="noopener noreferrer" :aria-label="`開啟 ${item.name} 的 Yahoo 股市報價`">{{ item.name }}</a><span v-else>{{ item.name }}</span></td><td class="fund_price"><strong>TWD {{ formatPrice(item.price) }}</strong><small class="fund_previous_close">前收盤 TWD {{ formatPrice(item.previousClose) }}</small></td><td class="fund_change"><strong :class="['fund_today_change', getChangeClass(item.changePct)]"><span>{{ isYahooQuoteVerified(item, activeQuote.savedAt) ? formatPercent(item.changePct) : '待確認' }}</span><em v-if="isFlatQuote(item, activeQuote.savedAt)" class="fund_flat_badge">平盤</em></strong><small v-if="isYahooQuoteVerified(item, activeQuote.savedAt)" :class="getChangeClass(getQuoteChangeAmount(item))">{{ formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else class="fund_flat">資料待驗證</small></td><td class="fund_weight"><strong>{{ item.weight.toFixed(2) }}%</strong></td></tr></tbody>
						</table>
					</div>
					<div class="fund_mobile_holdings" aria-label="官方公開前十大持股">
						<article class="fund_mobile_holding" v-for="item in activeFund.holdings" :key="'mobile-' + item.name"><div class="fund_mobile_holding_top"><div class="fund_mobile_company_group"><span class="fund_rank">{{ item.rank }}</span><a v-if="getYahooQuoteUrl(item)" class="fund_company_link" :href="getYahooQuoteUrl(item)" target="_blank" rel="noopener noreferrer" :aria-label="`開啟 ${item.name} 的 Yahoo 股市報價`">{{ item.name }}</a><strong v-else>{{ item.name }}</strong></div><div class="fund_mobile_change"><strong :class="getChangeClass(item.changePct)"><span>{{ isYahooQuoteVerified(item, activeQuote.savedAt) ? formatPercent(item.changePct) : '待確認' }}</span><em v-if="isFlatQuote(item, activeQuote.savedAt)" class="fund_flat_badge">平盤</em></strong><small v-if="isYahooQuoteVerified(item, activeQuote.savedAt)" :class="getChangeClass(getQuoteChangeAmount(item))">{{ formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else class="fund_flat">資料待驗證</small></div></div><div class="fund_mobile_holding_detail"><div class="fund_mobile_price fund_mobile_price_inline"><strong>TWD {{ formatPrice(item.price) }}</strong><small class="fund_previous_close">前收盤 TWD {{ formatPrice(item.previousClose) }}</small><small>比重 {{ item.weight.toFixed(2) }}%</small></div></div></article>
					</div>
