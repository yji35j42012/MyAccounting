		...
		<section class="fund_hero normal_shadow">
			<div class="fund_hero_main">
				...
				<div class="fund_hero_nav">
					<div class="fund_nav_label_row"><span class="fund_meta_label">最新公開淨值</span><button type="button"
					...
					activeNav.navUpdatedAt }}</small></div>
			...
			<aside class="fund_hero_meta fund_hero_contribution_card">
				...
				<section class="fund_batch_refresh_panel" aria-label="四檔基金資料更新狀態">
					<div class="fund_batch_refresh_head"><span>四檔基金資料更新</span><button type="button"
							class="fund_batch_refresh_button" :disabled="batchRefresh.isRefreshing"
							@click="clearAndRefreshAllFundsData" aria-label="清除四檔基金全部快取並重新取得資料"><svg
								...
								viewBox="0 0 24 24" aria-hidden="true">
								<path d="M20 11a8 8 0 1 0 2.34 5.66M20 4v7h-7" />
							</svg><span>{{ batchRefresh.isRefreshing ? `正在更新 ${batchRefresh.currentFundName}` :
					'一鍵重整四檔基金' }}</span></button></div><small class="fund_batch_refresh_summary"
						:class="{ 'is-error': batchRefresh.isError }" role="status">{{ batchRefreshStatus }}</small>
					<article v-for="row in batchRefreshRows" :key="row.key"
						:class="['fund_batch_refresh_row', row.isError ? 'is-error' : '']"><strong>{{ row.name
							}}</strong><span>淨值 {{ row.navUpdatedAt }} · 歷史 {{ row.historyUpdatedAt }}</span><span>持股 {{
					row.holdingsUpdatedAt }} · 股價 {{ row.quoteUpdatedAt }}</span><small>{{ row.statusText
							}}</small></article>
				</section>
			</aside>
		</section>
			...
			<div class="fund_section_head">
				...
				<div class="fund_holdings_controls">
					...
					<button type="button" class="fund_holdings_cache_clear_button"
						...
						@click="clearActiveFundHoldingsCache" aria-label="一鍵清除持股快取">{{ activeHoldings.isRefreshing ||
					activeQuote.isRefreshing ? '重新取得中' : '一鍵清除持股快取' }}</button>
				</div>
			...
			<div class="fund_table_box">
				<table class="fund_table">
					...
					item.name }}</span></td>
							...
							<td class="fund_change"><strong
									:class="['fund_today_change', getChangeClass(item.changePct)]"><span>{{
					...
					formatQuoteChange(getQuoteChangeAmount(item)) }}</small><small v-else
									class="fund_flat">資料待驗證</small></td>
...
<script>
const FUND_ANALYSIS_VERSION = 'fund-analysis-v1.5.34-2026.08.20';
...
module.exports = {
	data() {
		return {
			...
			batchRefresh: { isRefreshing: false, isError: false, currentFundName: '', message: '', completedAt: '', results: {} },
			funds: [
				{
	...
	computed: {
		...
		batchRefreshRows() { return this.funds.map(fund => { const result = this.batchRefresh.results?.[fund.key] || {}; const isError = result.success === false; const completedAt = result.completedAt || ''; const message = result.message || '尚未批次重整'; return { key: fund.key, name: fund.shortName, navUpdatedAt: this.navsByFund[fund.key]?.navUpdatedAt || '尚未完成', historyUpdatedAt: this.historiesByFund[fund.key]?.historyUpdatedAt || '尚未完成', holdingsUpdatedAt: this.holdingsByFund[fund.key]?.holdingsUpdatedAt || '尚未完成', quoteUpdatedAt: this.quotesByFund[fund.key]?.quoteUpdatedAt || '尚未完成', completedAt, isError, message, statusText: isError ? `${ message }${ completedAt ? `（本次完成 ${ completedAt }）` : '' }` : completedAt ? `本次完成 ${ completedAt }` : message }; }); },
		batchRefreshStatus() { if (this.batchRefresh.isRefreshing) return `正在清除快取並更新 ${ this.batchRefresh.currentFundName }`; if (this.batchRefresh.message) return this.batchRefresh.message; return '可一鍵清除四檔基金的本機快取並同步更新所有資料'; },
	...
	methods: {
		...
		getFundStorageKey(type, fundKey) { const version = type === 'quotes' ? 'v2' : type === 'holdings-signal' ? 'v3' : 'v1'; return `cashflow-manager:fund-${ type }:${ version }:${ fundKey }`; },
		...
		async clearAndRefreshAllFundsData() { const batchState = this.batchRefresh; if (batchState.isRefreshing) return false; batchState.isRefreshing = true; batchState.isError = false; batchState.currentFundName = ''; batchState.message = '正在清除四檔基金快取並重新取得資料'; batchState.results = {}; try { const outcomes = await Promise.all(this.funds.map(async fund => { batchState.currentFundName = fund.shortName; const success = await this.clearAndRefreshFundData(fund.key); return [fund.key, { success, completedAt: this.formatQuoteTime(Date.now()), message: success ? '已完成所有資料更新' : '部分資料更新失敗，已保留前次成功資料' }]; })); batchState.results = Object.fromEntries(outcomes); const failedCount = outcomes.filter(([, result]) => !result.success).length; batchState.isError = failedCount > 0; batchState.completedAt = this.formatQuoteTime(Date.now()); batchState.message = failedCount ? `四檔基金已完成重整；${ failedCount } 檔有部分資料更新失敗` : '四檔基金快取已清除，所有淨值、持股與 Yahoo 報價皆已更新'; return failedCount === 0; } finally { batchState.isRefreshing = false; batchState.currentFundName = ''; } },
		async clearActiveFundHoldingsCache() { const fundKey = this.activeFundKey; const targetFund = this.funds.find(fund => fund.key === fundKey); const quoteState = this.quotesByFund[fundKey]; const holdingsState = this.holdingsByFund[fundKey]; const signalState = this.holdingsSignalsByFund[fundKey]; if (holdingsState?.isRefreshing || quoteState?.isRefreshing) return false;['holdings', 'quotes', 'holdings-signal'].forEach(type => { try { localStorage.removeItem(this.getFundStorageKey(type, fundKey)); } catch { } }); if (targetFund) targetFund.holdings = targetFund.holdings.map(holding => ({ ...holding, price: null, previousClose: null, priceChange: null, changePct: null })); if (quoteState) { quoteState.quoteUpdatedAt = ''; quoteState.savedAt = 0; quoteState.quotedCount = 0; quoteState.cacheMode = 'cleared'; quoteState.quoteError = ''; } if (holdingsState) { holdingsState.cacheMode = 'cleared'; holdingsState.holdingsError = ''; } if (signalState) { signalState.weightedChangePct = null; signalState.fundContributionPct = null; signalState.totalWeight = 0; signalState.quotedCount = 0; signalState.holdingsCount = targetFund?.holdings.length || 0; signalState.quoteUpdatedAt = ''; signalState.savedAt = 0; signalState.cacheMode = 'cleared'; } const holdingsUpdated = await this.refreshHoldings(true, fundKey); const coverage = this.getQuoteCacheCoverage(fundKey); if (!holdingsUpdated) return false; if (coverage.quotedCount < coverage.expectedCount) await this.refreshYahooQuotes(fundKey); return !this.quotesByFund[fundKey]?.quoteError; },
		...
		applyHoldingsSnapshot(fundKey, snapshot, cacheMode = 'remote') {
			...
			if (holdingsChanged) {
				...
				if (cacheMode !== 'local') { try { localStorage.removeItem(this.getFundStorageKey('quotes', fundKey)); localStorage.removeItem(this.getFundStorageKey('holdings-signal', fundKey)); } catch { } }
