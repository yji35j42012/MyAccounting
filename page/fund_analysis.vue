...
<template>
    <div class="fund">
        ...
        <section class="fund_hero normal_shadow">
            ...
            <div class="fund_hero_meta">
                ...
                <a class="fund_source_link" :href="activeFund.sourceUrl" target="_blank"
                    rel="noopener noreferrer">查看官方資料</a>
            </div>
        </section>
        ...
        <section class="fund_history normal_shadow">
            ...
            <p class="fund_history_source">資料來源：<a :href="activeFund.sourceUrl + '?nav=navperformance'" target="_blank"
                    rel="noopener noreferrer">安聯投信淨值走勢</a>；近期逐日資料以<a :href="activeFund.historySourceUrl" target="_blank"
                    rel="noopener noreferrer">公開基金淨值表</a>交叉核對。</p>
        </section>

        <section class="fund_holdings normal_shadow">
            <div class="fund_section_head">
                <div>
                    <p class="fund_kicker">投資標的</p>
                    <h3>官方公開前十大持股</h3>
                </div>
                ...
                <table class="fund_table">
                    ...
                    <tbody>
                        <tr v-for="item in activeFund.holdings" :key="item.name">
                            ...
                            <td class="fund_company">{{ item.name }}</td>
                            <td class="fund_price"><strong>TWD {{ formatPrice(item.price) }}</strong><small>{{
                    item.market }}</small></td>
                            <td><span :class="['fund_today_change', getChangeClass(item.changePct)]">{{
                    formatPercent(item.changePct) }}</span></td>
                            <td class="fund_weight">
                                <div class="fund_weight_value"><strong>{{ item.weight.toFixed(2) }}%</strong></div>
                                <div class="fund_progress"><span
                                        :style="{ width: (item.weight / maxWeight * 100) + '%' }"></span></div>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="fund_mobile_holdings" aria-label="官方公開前十大持股">
                <article class="fund_mobile_holding" v-for="item in activeFund.holdings" :key="'mobile-' + item.name">
                    <div class="fund_mobile_holding_top">
                        <div class="fund_mobile_company_group"><span class="fund_rank">{{ item.rank }}</span><strong>{{
                    item.name }}</strong></div>
                        <div class="fund_mobile_change"><strong :class="getChangeClass(item.changePct)">{{
                    formatPercent(item.changePct) }}</strong><small>今日漲跌幅</small></div>
                    </div>
                    <div class="fund_mobile_holding_detail">
                        <div class="fund_mobile_price fund_mobile_price_inline"><strong>TWD {{ formatPrice(item.price)
                                }}</strong><small>{{ item.market }} ・ 見上方報價時間</small></div>
                        <div class="fund_progress"><span
                                :style="{ width: (item.weight / maxWeight * 100) + '%' }"></span></div>
                    </div>
                </article>
        ...
        <section class="fund_notice">
            <p><strong>資料揭露：</strong>{{ activeFund.name }}的最新淨值與持股資料分別依安聯投信官方頁面及官方投資持股頁列示；¹
                個股價格會在進入本頁、盤中每五分鐘，以及按下「更新股價」時，經本網站伺服端代理自 Yahoo 股市讀取 regularMarketPrice；² 今日漲跌幅按「(Yahoo 當前報價 − 前一交易日收盤價)
                ÷ 前一交易日收盤價」計算。盤中報價可能與交易所當日最終收盤價不同；若更新失敗，頁面會保留該基金前次成功取得的報價。持股會因基金經理人調整而變動，畫面僅呈現官方公開列示標的，非完整投資組合。</p>
            <p><strong>用途說明：</strong>本頁為公開資料整理與持股結構觀察，不構成任何買賣、申購或贖回建議。</p>
        </section>
    </div>
</template>
...
module.exports = {
    ...
    mounted() { this.refreshYahooQuotes(); this.quoteTimer = window.setInterval(this.refreshYahooQuotes, 5 * 60 * 1000); store.dispatch('SET_LOADING_ACTION', false); },
    beforeUnmount() { if (this.quoteTimer) window.clearInterval(this.quoteTimer); }
};
</script>
