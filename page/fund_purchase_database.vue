	...
	<section class="fund_purchase_page fund_purchase_db_page">
		...
		<section v-if="!isLoading && !session" class="fund_purchase_page_content" aria-label="資料庫申購紀錄登入提示">
			<article class="fund_purchase_page_card normal_shadow">
				...
				<p class="fund_purchase_page_note">資料表已啟用資料列安全性規則；未登入時不會讀取、寫入或顯示任何資料庫資料。</p><router-link
					class="fund_purchase_page_add_button" to="/login">前往登入</router-link>
			</article>
		</section>
		...
		<template v-else>
			<header class="fund_purchase_page_hero normal_shadow">
				<div>
					...
					<p>資料庫帳號：{{ userEmail || '已登入' }}。已結算贖回採加權平均成本；處理中贖回僅保留單位。</p>
				</div>
				<div class="fund_purchase_page_refresh"><button class="fund_purchase_page_refresh_button" type="button"
						:disabled="isLoading || isRefreshingNavs" @click="refreshPageData"><svg
							class="fund_refresh_icon" :class="{ 'is-spinning': isLoading || isRefreshingNavs }"
							viewBox="0 0 24 24" aria-hidden="true">
							<path d="M20 11a8 8 0 1 0 2 5.5M20 4v7h-7"></path>
						</svg>{{ isLoading || isRefreshingNavs ? '正在更新資料庫與淨值' : '更新資料庫與淨值' }}</button>
					<p v-if="lastLoadedAt" class="fund_purchase_page_refresh_status">
						資料庫最後載入：{{ formatTaipeiDateTime(lastLoadedAt) }}
					</p>
			...
			<section class="fund_purchase_page_totals" aria-label="資料庫基金申購與贖回總覽">
				...
				<article class="fund_purchase_page_total fund_purchase_page_realized_total normal_shadow">
					<span>累積已實現損益</span><strong :class="getChangeClass(activeLedger.realizedProfitLoss)">{{
						formatSignedTwd(activeLedger.realizedProfitLoss) }}</strong><small>僅計入已結算贖回</small>
				</article>
				...
				<article class="fund_purchase_page_card normal_shadow">
					<header class="fund_purchase_page_card_head">
						<div>
							...
							<div class="fund_purchase_page_tabs" role="tablist" aria-label="資料庫申購、贖回與庫存總覽"><button
					...
					<section v-if="activeTab === 'purchase'" aria-label="資料庫申購紀錄">
						<div class="fund_purchase_page_card_controls">
							<button class="fund_purchase_page_add_button" type="button"
								@click="openPurchaseModal('add')">＋ 新增申購紀錄
							</button>
							...
							<table class="fund_purchase_page_table">
								...
								<tbody>
									<tr v-if="visibleRecords.length === 0">
										<td colspan="8" class="fund_purchase_page_empty">
											{{ isLoading ? '正在讀取資料庫資料…' : '資料庫目前沒有符合篩選條件的申購紀錄。' }}
										</td>
									</tr>
									<tr v-for="record in visibleRecords" :key="record.id">
										<td>{{ formatDate(record.date) }}
											<span v-if="record.isIncomplete" class="fund_purchase_page_pending_badge">
												待補資料
											</span>
										</td>
										...
										<td>
											<strong :class="getChangeClass(record.returnPct)">
												{{ formatPercent(record.returnPct) }}
											</strong>
										</td>
										<td>
											<strong :class="getChangeClass(record.profitLoss)">
												{{ formatSignedTwd(record.profitLoss) }}
											</strong>
										</td>
										<td>
											<button class="fund_purchase_page_edit_button" type="button"
												:aria-label="`編輯 ${formatDate(record.date)} 的資料庫申購紀錄`"
												@click="openPurchaseModal('edit', record)">
												編輯
											</button>
										</td>
									</tr>
					...
					<section v-else-if="activeTab === 'redemption'" aria-label="資料庫贖回紀錄">
						<div class="fund_purchase_page_redemption_intro">
							<div><strong>可再贖回 {{ formatUnits(redemptionAvailableUnits) }}</strong><span>已扣除 {{
								formatUnits(activeLedger.pendingUnits) }} 處理中保留單位</span></div><button
								class="fund_purchase_page_add_button fund_purchase_page_redemption_add_button"
								type="button" @click="openRedemptionModal('add')">＋ 新增贖回紀錄</button>
						</div>
						<div class="fund_purchase_page_table_box">
								...
								<tbody>
									...
									<tr v-for="record in activeRedemptionRecords" :key="record.id">
										...
										<td><button class="fund_purchase_page_edit_button" type="button"
												:aria-label="`編輯 ${formatDate(record.date)} 的資料庫贖回紀錄`"
							...
							<article v-for="record in activeRedemptionRecords" :key="`redemption-mobile-${record.id}`"
								...
								<dl>
									...
									<div>
										...
										<dd>
											{{ record.status === 'settled' ?
												formatTwd(getRedemptionMetric(record).netProceeds) : '—' }}
										</dd>
									</div>
									<div>
										<dt>成本基礎</dt>
									...
									class="fund_purchase_page_edit_button fund_purchase_page_mobile_edit_button"
					...
					<section v-else class="fund_purchase_page_inventory" aria-label="資料庫庫存總覽">
						...
						<p class="fund_purchase_page_inventory_note">
							加權平均成本：每筆已結算贖回會以該筆交易日期前、資料完整的尚餘申購部位計算平均成本。處理中交易只保留單位，不計入損益；取消交易不影響庫存與損益。</p>
					</section>
				</article>
			</section>
		</template>
		...
		<div :class="['alert', 'fund_purchase_page_modal', purchaseModal.mode ? 'showAdd' : '']" role="dialog"
			aria-modal="true" aria-labelledby="fund-purchase-db-modal-title" @click.self="closePurchaseModal">
			<div class="alert_box"><button class="alert_close" type="button" aria-label="關閉資料庫申購紀錄視窗"
				...
				<form class="alert_content" @submit.prevent="saveDatabaseRecord">
					...
					<div class="alert_content_item" data-txt="投入本金"><input v-model="purchaseModal.form.principal"
							class="alert_inp" type="number" min="0.01" step="0.01" placeholder="請輸入投入本金" required
					...
					<div class="alert_content_item" data-txt="庫存單位數（選填）"><input v-model="purchaseModal.form.units"
							...
							placeholder="例如 250.47；最多小數點後兩位" :disabled="isSaving"></div>
					<p v-if="purchaseModal.error" class="fund_purchase_page_modal_error" role="alert">{{
						purchaseModal.error }}
					</p>
					<p class="fund_purchase_page_modal_hint">此頁只寫入登入帳號自己的 Supabase 申購資料；留白欄位會儲存為資料庫
						`NULL`，並顯示為待補資料。庫存單位數最多輸入至小數點後兩位。</p>
					<div class="alert_funcbox">
						<button class="normal_btn _secondary" type="button" :disabled="isSaving"
							@click="closePurchaseModal">取消</button>
						<button class="normal_btn _primary" type="submit" :disabled="isSaving">
							{{ isSaving ? '儲存中…' : purchaseModal.mode === 'edit' ? '儲存更新' : '新增紀錄' }}
						</button>
					</div>
				</form>
			</div>
		...
		<div :class="['alert', 'fund_purchase_page_modal', redemptionModal.mode ? 'showAdd' : '']" role="dialog"
			aria-modal="true" aria-labelledby="fund-redemption-db-modal-title" @click.self="closeRedemptionModal">
			<div class="alert_box"><button class="alert_close" type="button" aria-label="關閉資料庫贖回紀錄視窗"
					@click="closeRedemptionModal"><svg viewBox="0 0 24 24" aria-hidden="true">
						<path d="M18 6 6 18M6 6l12 12"></path>
					</svg></button>
				<div class="alert_title">
					<span id="fund-redemption-db-modal-title">
						{{ redemptionModal.mode === 'edit' ? '編輯資料庫贖回紀錄' : '新增資料庫贖回紀錄' }}
					</span>
				...
				<form class="alert_content" @submit.prevent="saveDatabaseRedemption">
					<div class="alert_content_item" data-txt="贖回日期"><input v-model="redemptionModal.form.date"
							class="alert_inp" type="date" required :disabled="isSaving"></div>
					<div class="alert_content_item" data-txt="狀態">
						<select v-model="redemptionModal.form.status" class="alert_inp" :disabled="isSaving">
							<option value="pending">處理中</option>
							<option value="settled">已結算</option>
							<option value="cancelled">取消</option>
						</select>
					</div>
					<div class="alert_content_item" data-txt="贖回單位數">
						<input v-model="redemptionModal.form.units" class="alert_inp" type="number" min="0.01"
							step="0.01" inputmode="decimal" placeholder="可再贖回單位數" required :disabled="isSaving">
					</div>
					<div class="alert_content_item" data-txt="贖回淨值（已結算必填）">
						<input v-model="redemptionModal.form.redemptionNav" class="alert_inp" type="number" min="0.01"
							step="0.01" placeholder="基金公司正式贖回淨值"
							:disabled="isSaving || redemptionModal.form.status !== 'settled'">
					</div>
					<div class="alert_content_item" data-txt="手續費">
						<input v-model="redemptionModal.form.fee" class="alert_inp" type="number" min="0" step="0.01"
							placeholder="未收費請填 0" :disabled="isSaving">
					</div>
					<div class="alert_content_item" data-txt="稅費／其他扣款">
						<input v-model="redemptionModal.form.tax" class="alert_inp" type="number" min="0" step="0.01"
							placeholder="未扣款請填 0" :disabled="isSaving">
					</div>
					<div class="alert_content_item" data-txt="備註">
						<input v-model="redemptionModal.form.note" class="alert_inp" type="text" maxlength="100"
							placeholder="選填，例如入帳銀行或交易單號" :disabled="isSaving">
					</div>
					...
					<p class="fund_purchase_page_modal_hint">
						處理中交易會保留單位；已結算交易才會扣除庫存並計入已實現損益；取消交易不影響庫存與損益。贖回單位數最多輸入至小數點後兩位。</p>
					<div class="alert_funcbox">
						<button class="normal_btn _secondary" type="button" :disabled="isSaving"
							@click="closeRedemptionModal">取消</button>
						<button class="normal_btn _primary" type="submit" :disabled="isSaving">{{ isSaving ? '儲存中…' :
							redemptionModal.mode === 'edit' ? '儲存更新' : '儲存贖回紀錄' }}</button>
					</div>
				</form>
			</div>
		</div>
		...
		<p v-if="session" class="fund_purchase_page_note">申購未實現損益：市值＝尚餘庫存單位數 × 最新公開淨值；已結算贖回：入帳淨額＝贖回單位數 ×
			贖回淨值－手續費－稅費／其他扣款，並以加權平均成本計算已實現損益。</p>
	</section>
</template>

<script>
const FUND_PURCHASE_DATABASE_PAGE_VERSION = 'fund-purchase-db-v1.1.1-2026.08.27';
...
module.exports = {
	data() {
		return {
			...
			activeTab: 'purchase',
			recordTabs: [
				{ key: 'purchase', label: '申購紀錄' },
				{ key: 'redemption', label: '贖回紀錄' },
	...
	computed: {
		...
		activeIncompleteRecordCount() {
			return this.activeRecords.filter(record => record.isIncomplete).length;
		},
		activeRedemptionRecords() {
			return [...this.redemptions.filter(record => record.fundKey === this.activeFundKey)].sort((left, right) => this.normalizeDate(right.date).localeCompare(this.normalizeDate(left.date)));
		},
		activeLedger() {
			return this.calculateFundLedger(this.activeFundKey);
		},
		redemptionAvailableUnits() {
			return this.getRedeemableUnitsForModal();
		},
		...
		activeNavStatus() {
			...
			return this.activeFund.navUpdatedAt ? `最新公開淨值已更新：${this.activeFund.navUpdatedAt}` : '尚未更新，將使用已保存的淨值資料';
		},
	},
	async mounted() {
		...
		this.authChangeHandler = event => {
			...
			const eventName = event.detail?.event || ''; this.session = nextSession;
			...
			if (!nextSession) {
				this.records = []; this.redemptions = []; this.pageError = '';
				return;
			}
	...
	methods: {
		async bootstrap() {
			...
			try {
				...
				this.redemptions = [];
				this.pageError =
					this.getFriendlyError(error, '無法初始化資料庫申購與贖回紀錄。');
			} finally {
		...
		formatDate(value) {
			...
			return date ? date.replace(/-/g, ' / ') : '尚未取得';
		},
		formatTime(value) {
			const match = String(value || '').match(/(\d{1,2}:\d{2})/);
			return match ? match[1] : '尚未更新';
		},
		formatNav(value) {
			return this.isUsableNumber(value) && Number(value) > 0 ? `${Number(value).toLocaleString('en-US', {
				minimumFractionDigits: 2, maximumFractionDigits: 2
			})} 新臺幣` : '—';
		},
		formatTwd(value) {
			return this.isUsableNumber(value) ? `TWD ${Number(value).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'TWD —';
		},
		formatSignedTwd(value) {
			return this.isUsableNumber(value) ? `${Number(value) > 0 ? '+' : Number(value) < 0 ? '-' : ''}TWD ${Math.abs(Number(value)).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'TWD —';
		},
		formatUnits(value) {
			return this.isUsableNumber(value) ? `${Number(value).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} 單位` : '—';
		},
		formatPercent(value) {
			return this.isUsableNumber(value) ? `${Number(value) > 0 ? '+' : ''}${Number(value).toFixed(2)}%` : '—';
		},
		formatRedemptionStatus(status) {
			return ({ pending: '處理中', settled: '已結算', cancelled: '取消' })[status] || '處理中';
		},
		formatTaipeiDateTime(timestamp) {
			return new Intl.DateTimeFormat('zh-TW', { timeZone: 'Asia/Taipei', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(timestamp)).replace(/\//g, ' / ').replace(',', '');
		},
		getChangeClass(value) {
			return Number(value) > 0 ? 'fund_positive' : Number(value) < 0 ? 'fund_negative' : 'fund_flat';
		},
		getFriendlyError(error, fallback) {
			const message = String(error?.message || '');
			if (/JWT|session|not authenticated|401|403/i.test(message))
				return '登入工作階段已失效或沒有資料庫權限，請重新登入後再試一次。';
			if (/fund_redemption_records|schema cache|relation/i.test(message))
				return '贖回資料表尚未建立。請先在 Supabase SQL Editor 執行贖回資料表設定 SQL，再重新整理此頁。';
